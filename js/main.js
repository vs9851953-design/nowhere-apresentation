/* ==========================================================================
   ...
   e das pastas de /gallery — o index.html não contém texto nem caminhos
   de imagem além dos ativos fixos de estrutura (logo/hero/favicon).

   Ordem de execução:
   1. Carrega presentation.json
   2. Resolve as imagens de cada pasta da galeria (automático)
   3. Constrói os slides 1–8
   4. Inicia o SlideController
   5. Ao chegar na última etapa, constrói a página de revisão (etapa 9)
...
(async function bootstrap() {
  ...
  const galeria = await fetchAllGalleryImages(data.secoes);
  ...
  SECTION_ORDER.forEach(chave => {
    ...
    wrap.appendChild(renderer(galeria[chave], secao.pasta));
